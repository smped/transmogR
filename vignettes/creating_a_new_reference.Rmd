...
have been included as well as some checking steps.
Variant callers can often produce variant calls which are incompatible and the
function `checkVariants()` will check for overlapping InDels, as well as 
returning the `ALT` column as a character vector.

```{r clean-variants}
var <- cleanVariants(var)
```

In this case, no incompatible variants were included, but if they are present, 
an error will be proudced by default.
This behaviour can be modified by setting the `ol_vars` argument to return no
overlapping InDels (`ol_vars = 'none'`), the longest or shortest in terms of 
the largest or smallest change to the chromosome length, or even the first or 
last of the overlapping variants.
...
In addition to the above approaches, the co-ordinates from a GRanges object can
...
```{r}
alt_trans_mod <- extractTranscriptSeqs(
    chr1_mod, splitAsList(new_exons, new_exons$transcript_id)
)
```

In this case tags may have to be added manually

```{r}
ol_ids <- var %>% 
    splitAsList(varTypes(.)) %>% 
    lapply(\(x) subsetByOverlaps(gtf$exon, x)$transcript_id) %>% 
    lapply(\(x) names(alt_trans_mod) %in% x)
tag_mat <- do.call(
    "cbind", 
    lapply(
        c("SNV", "Insertion", "Deletion"),
        \(x) ifelse(ol_ids[[x]], tolower(substring(x, 1, 1)), "")
    ) 
)
tags <- gsub("([sid]+)", "_mod_\\1", apply(tag_mat, 1, paste, collapse = ""))
names(alt_trans_mod) <- paste0(names(alt_trans_mod), tags)
