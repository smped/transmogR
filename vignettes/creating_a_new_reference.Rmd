...
`mogrifyGenome()` with either a `DNAStringSet` or `BSgenome` object.
A tag can be optionally added to all sequence names to avoid confusion.

```{r chr1-mod}
chr1_mod <- mogrifyGenome(chr1, var, tag = "mod")
chr1_mod
```
...
```{r trans-mod}
trans_mod <- mogrifyTranscriptome(
...
be passed to `mogrifyTranscriptome()` and any trascripts which overlap the PAR-Y
region will be excluded.
Alternatively, passing the entire Y-chromosome to `mogrifyTranscriptome()` will 
